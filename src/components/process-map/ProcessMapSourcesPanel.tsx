...
export function ProcessMapSourcesPanel() {
  const { sources, addSource, removeSource } = useProcessMapStore();
  ...
  return (
    <div className="section-card overflow-hidden">
      <div className="px-4 py-3 section-card-header">
        <h2 className="text-sm font-semibold text-[var(--text)]">Source documents</h2>
        <p className="text-xs text-[var(--text-muted)] mt-0.5">
          Interview notes, transcripts, SOPs, and process docs
        </p>
      </div>
      <div className="p-4">
        <button
        ...
        <div className="mt-4 space-y-3">
          {sources.length === 0 && (
            <p className="text-xs text-[var(--text-muted)] italic">
              No files uploaded. BSN seeds and workflow context still apply.
            </p>
          )}
          {sources.map((s) => (
            <div
